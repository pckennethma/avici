...
if __name__ == "__main__":
    ...
    g, x, interv = simulate_data(d=10, n=300, n_interv=0, path="ldp/config/ldp_rlc_sparse.yaml")
    # g, x, interv = simulate_data(d=20, n=300, n_interv=0, domain="lin-gauss")
    
    
    model = avici.load_pretrained(download="neurips-linear")
    # ldp_model = avici.load_pretrained(checkpoint_dir="ldp/checkpoints/rlc-sparse", expects_counts=False)
    ldp_model = avici.load_pretrained(checkpoint_dir="/scratch/trllmout/pmaab/avici/checkpoint", expects_counts=False)
